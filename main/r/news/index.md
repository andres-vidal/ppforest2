...
- Fixed a `-Wdeprecated-declarations` warning reported by CRAN’s
  ...
  `std::basic_ostream<std::uint8_t>`). ppforest2 does not use nlohmann’s
  binary formats, so the vendored `json.hpp` is now bracketed with a
  `_Pragma` guard that suppresses the deprecation. `_Pragma` (unlike
  `#pragma`) is not flagged by `R CMD check`’s pragma check. The guard
  is applied by `make r-vendor-deps` (`scripts/vendor-guard-json.sh`).
